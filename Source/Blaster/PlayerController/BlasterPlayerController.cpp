...
{
	// can now calculate Round Trip Time
...
void ABlasterPlayerController::UpdateAnnouncement()
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	...
	if (bHUDValid)
	{
		...
		if (BlasterGameState && BlasterPlayerState)
		{
			...
			{
				InfoTextString = FString::Printf(TEXT("Top player: \n%s: %d"), *TopPlayers[0]->GetPlayerName(), static_cast<int>(TopPlayers[0]->GetScore()));
			}
			else if (TopPlayers.Num() > 1)
			{
				InfoTextString = FString("Top players tied for the win:\n");
				for (auto TiedPlayer : TopPlayers)
				{
					InfoTextString.Append(FString::Printf(TEXT("%s: %d\n"), *TiedPlayer->GetPlayerName(), static_cast<int>(TiedPlayer->GetScore())));
				}
			}
...
{
	APlayerState* Self = GetPlayerState<APlayerState>(); // If I want to show diff msg if self is involved
